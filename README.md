<div align="center">




<img src="https://readme-typing-svg.demolab.com?font=Fira+Code&size=20&duration=3000&pause=900&color=A855F7&center=true&vCenter=true&width=650&lines=Building+intelligent+systems+from+data;AI+%7C+Generative+AI+%7C+LLMs;Full-Stack+%2B+Applied+AI+Engineer" alt="Typing SVG" />

<br/>

<img src="https://img.shields.io/badge/AI%20Engineering-6C63FF?style=flat-square&logoColor=white"/>
<img src="https://img.shields.io/badge/Generative%20AI-7C5CFC?style=flat-square&logoColor=white"/>
<img src="https://img.shields.io/badge/LLMs-8D52E8?style=flat-square&logoColor=white"/>
<img src="https://img.shields.io/badge/Data%20Engineering-9D45D9?style=flat-square&logoColor=white"/>
<img src="https://img.shields.io/badge/Full%20Stack%20Developer-A855F7?style=flat-square&logoColor=white"/>
<br/>
<p>
  <a href="https://zeinebghrab.vercel.app/" target="_blank"><img src="https://img.shields.io/badge/🌐_Portfolio-6C63FF?style=for-the-badge&logoColor=white"/></a>
  <a href="https://www.linkedin.com/in/zeineb-ghrab/" target="_blank"><img src="https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white"/></a>
</p>

</div>


<img src="https://capsule-render.vercel.app/api?type=rect&color=0:6C63FF,100:A855F7&height=3&width=100%"/>



## 👩‍💻 About me

<table>
<tr>
<td>

📚 I am a Data & Decisional Systems Engineering student at ENET'Com, passionate about building intelligent systems that transform data into actionable insights.

- 🎓 Valedictorian in Data & Decisional Systems Engineering
- 🎓 Valedictorian in Computer Science (Big Data & Data Analysis)
- 🧠 Passionate about AI, Data Science, LLMs, and Generative AI
- ⚙️ Experienced in building end-to-end AI systems (ASR · LLM · TTS · Computer Vision)
- 💻 Full-stack developer (React, FastAPI, NestJS, PHP)
- 📊 Interested in Decision Systems, Optimization, and Applied AI

</td>
</tr>
</table>


## 🚀 Projects

<table>
<tr>
<td width="50%" valign="top">

### ⚡ [SparkHire AI](https://github.com/ZeinebGhrab/SparkHire-AI)
**Intelligent Vocal Interview Platform**

Developed an AI-powered platform for automated voice-based interviews with real-time evaluation using speech recognition, LLMs, and computer vision, generating structured scoring and decision-support reports.

`Python` `FastAPI` `MongoDB` `WebSocket` `PySide6` `Whisper` `Llama 3.2` `Edge-TTS` `MediaPipe` `HSEmotion`

</td>
<td width="50%" valign="top">

### 📚 [EduPlanner](https://github.com/ZeinebGhrab/EduPlanner)
**Intelligent Scheduling & Decision Support Platform**

Designed and implemented an intelligent decision-support system for training center planning, combining constraint-based scheduling, optimization heuristics, and real-time conflict resolution within a secure full-stack web application.

`Java` `Spring Boot` `Optimization Algorithms` `Decision Systems` `JWT` `JavaScript (ES6)` `HTML5` `CSS3`

</td>
</tr>

<tr>
<td width="50%" valign="top">

### 🚗 [AlloCovoit](https://github.com/ZeinebGhrab/AlloCovoit)
**Carpooling Web Platform**

Developed a modern full-stack carpooling web application allowing users to publish trips, manage reservations, and connect securely with other travelers. The platform includes user and admin dashboards, real-time updates via AJAX, email notifications, and secure authentication with role-based access control.

`PHP` `MySQL` `JavaScript (ES6)` `HTML5` `CSS3` `AJAX` `PHPMailer` `Composer`

</td>
<td width="50%" valign="top">

### 🚀 [Aurora](https://github.com/ZeinebGhrab/Aurora)
**Next-Generation Educational Platform**

Developed a modern full-stack web application for educational management, enabling administrators, teachers, and students to manage courses, sessions, and attendance. Includes real-time updates, email notifications, and facial recognition for attendance verification. Supports course planning, session scheduling, and detailed student analytics.

`PHP` `MySQL` `JavaScript (ES6)` `HTML5` `CSS3` `PHPMailer` `Composer` `Python (DeepFace)`

</td>
</tr>

<tr>
<td width="50%" valign="top">

### 🧠 [Stroke Clinical Data Statistical Analysis](https://github.com/ZeinebGhrab/Stroke-Clinical-Data-Statistical-Analysis)

Performed statistical analysis on stroke clinical data to investigate associations between NIHSS scores, age, lifestyle factors, and stroke presence, integrating visualizations and hypothesis testing for medical insights.

`Python` `Pandas` `SciPy` `Seaborn` `Matplotlib`

</td>
<td width="50%" valign="top">

### ⚙️ [MiniGit](https://github.com/ZeinebGhrab/MiniGit)
**Git-inspired lightweight version control system for Linux**

Built a minimalist VCS in C for Linux, featuring SHA-256 file hashing, blob creation, WorkTree management, and filesystem snapshots.

`C` `Linux` `GCC`

</td>
</tr>

<tr>
<td width="50%" valign="top">

### 🌐 [IDSD Website](https://github.com/ZeinebGhrab/IDSD-website)

Developed an IDSD showcase website presenting the Data Engineering and Decision Systems program at ENET'Com, with student profiles and career opportunities.

`HTML5` `CSS3` `XML` `XSLT` `XPath` `XQuery`

</td>
<td width="50%" valign="top">

### 📚 [BookMatch AI](https://github.com/ZeinebGhrab/BookMatch-AI)
**AI-Powered Book Recommendation System**

Built a smart book recommendation platform powered by an AI agent that personalizes suggestions based on user preferences.

`Flask` `React.js` `TypeScript` `Bootstrap` `Python`

</td>
</tr>

<tr>
<td width="50%" valign="top">

### 🧹 [CleanSim App](https://github.com/ZeinebGhrab/CleanSim-App)
**AI-Driven Cleaning Simulation Platform**

Developed a web-based simulation featuring an autonomous AI agent capable of navigating and cleaning a dynamically generated environment.

`Flask` `React.js` `TypeScript` `Bootstrap` `Python`

</td>
<td width="50%" valign="top">

### 📦 [InvenPro](https://github.com/ZeinebGhrab/InvenPro)
**Inventory Management Web Application**

Designed and built a full-stack web application for managing inventory data efficiently.

`Flask` `React.js` `TypeScript` `Bootstrap` `Python`

</td>
</tr>

<tr>
<td width="50%" valign="top">

### 🧾 [Princo](https://github.com/ZeinebGhrab/princo)
**Automated Receipt Printing System**

Developed a complete solution consisting of a web platform and a desktop application to automate receipt printing from an e-commerce platform.

`ReactJs` `ElectronJs` `NestJs`

</td>
<td width="50%" valign="top">

### 🧮 [Invoice Generator](https://github.com/ZeinebGhrab/Invoice_Generator)
**Invoice Generation Web Platform**

Designed and implemented a web application for generating, viewing, and downloading invoices.

`React.js` `Tailwind CSS` `PostgreSQL` `Strapi.js` `Handlebars.js`

</td>
</tr>

<tr>
<td width="50%" valign="top">

### 🌍 [Discover World](https://github.com/ZeinebGhrab/Discover_world)
**World Exploration Mobile App**

Designed and developed an immersive Android mobile application to promote global discovery through interactive features like quizzes and guessing games.

`Java` `XML` `Android Development`

</td>
<td width="50%" valign="top">

</td>
</tr>
</table>


## 🔧 My Tech Stack

<table width="100%">
<tr>
<td valign="top" width="33%">

<h4>🎨 Frontend</h4>

<img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/html5/html5-original.svg" height="40" title="HTML5"/>
<img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/css3/css3-original.svg" height="40" title="CSS3"/>
<img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/javascript/javascript-original.svg" height="40" title="JavaScript"/>
<img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/typescript/typescript-original.svg" height="40" title="TypeScript"/>
<img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/react/react-original.svg" height="40" title="React"/>
<img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/ionic/ionic-original.svg" height="40" title="Ionic"/>
<img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/electron/electron-original.svg" height="40" title="Electron"/>

</td>
<td valign="top" width="33%">

<h4>⚙️ Backend & APIs</h4>

<img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/nestjs/nestjs-original.svg" height="40" title="NestJS"/>
<img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/fastapi/fastapi-original.svg" height="40" title="FastAPI"/>
<img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/django/django-plain.svg" height="40" title="Django"/>
<img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/spring/spring-original.svg" height="40" title="Spring Boot"/>
<img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/flask/flask-original.svg" height="40" title="Flask"/>

</td>
<td valign="top" width="33%">

<h4>🗄️ Databases</h4>

<img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/mongodb/mongodb-original.svg" height="40" title="MongoDB"/>
<img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/postgresql/postgresql-original.svg" height="40" title="PostgreSQL"/>
<img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/mysql/mysql-original.svg" height="40" title="MySQL"/>

</td>
</tr>
<tr><td colspan="3"><br/></td></tr>
<tr>
<td valign="top" width="33%">

<h4>🧠 AI · Data Science · Computer Vision</h4>

<img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/python/python-original.svg" height="40" title="Python"/>
<img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/pytorch/pytorch-original.svg" height="40" title="PyTorch"/>
<img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/opencv/opencv-original.svg" height="40" title="OpenCV"/>
<img src="https://img.shields.io/badge/YOLO-111F68?style=for-the-badge&logoColor=white" height="28" title="YOLO"/>
<img src="https://img.shields.io/badge/MediaPipe-00B5A5?style=for-the-badge&logoColor=white" height="28" title="MediaPipe"/>
<img src="https://img.shields.io/badge/FaceMesh-FF6F61?style=for-the-badge&logoColor=white" height="28" title="FaceMesh"/>
<img src="https://img.shields.io/badge/DeepFace-3B82F6?style=for-the-badge&logoColor=white" height="28" title="DeepFace"/>
<img src="https://img.shields.io/badge/HSEmotion-EC4899?style=for-the-badge&logoColor=white" height="28" title="HSEmotion"/>
<img src="https://img.shields.io/badge/Whisper-10A37F?style=for-the-badge&logoColor=white" height="28" title="Whisper"/>

</td>
<td valign="top" width="33%">

<h4>🤖 Generative AI · LLMs</h4>

<img src="https://raw.githubusercontent.com/simple-icons/simple-icons/develop/icons/huggingface.svg" height="40" title="Hugging Face"/>
<img src="https://raw.githubusercontent.com/simple-icons/simple-icons/develop/icons/langchain.svg" height="40" title="LangChain"/>
<img src="https://img.shields.io/badge/Ollama-000000?style=for-the-badge&logoColor=white" height="28" title="Ollama"/>
<img src="https://img.shields.io/badge/Llama_3.2-1877F2?style=for-the-badge&logoColor=white" height="28" title="Llama 3.2"/>
<img src="https://img.shields.io/badge/RAG-A855F7?style=for-the-badge&logoColor=white" height="28" title="RAG"/>
<img src="https://img.shields.io/badge/Edge_TTS-22C55E?style=for-the-badge&logoColor=white" height="28" title="Edge-TTS"/>

</td>
<td valign="top" width="33%">

<h4>🛠️ Tools & DevOps</h4>

<img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/git/git-original.svg" height="40" title="Git"/>
<img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/github/github-original.svg" height="40" title="GitHub"/>
<img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/docker/docker-original.svg" height="40" title="Docker"/>
<img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/apachekafka/apachekafka-original.svg" height="40" title="Kafka"/>
<img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/figma/figma-original.svg" height="40" title="Figma"/>
<img src="https://img.shields.io/badge/n8n-EA4B71?style=for-the-badge&logoColor=white" height="28" title="n8n"/>

</td>
</tr>
</table>

## 📊 GitHub Stats

<div align="center">

<table width="100%">
<tr>
<td valign="top" width="50%">
<img width="100%" src="https://github-readme-stats.vercel.app/api?username=ZeinebGhrab&show_icons=true&theme=radical&hide_border=true&bg_color=ffffff&title_color=7C3AED&icon_color=A855F7&text_color=2d2d2d&border_radius=14"/>
</td>
<td valign="top" width="50%">
<img width="100%" src="https://github-readme-stats.vercel.app/api/top-langs/?username=ZeinebGhrab&layout=compact&theme=radical&hide_border=true&bg_color=ffffff&title_color=7C3AED&text_color=2d2d2d&border_radius=14"/>
</td>
</tr>
</table>

<img width="100%" src="https://streak-stats.demolab.com?user=ZeinebGhrab&hide_border=true&background=ffffff&ring=A855F7&fire=7C3AED&currStreakLabel=7C3AED&sideLabels=2d2d2d&sideNums=2d2d2d&dates=9ca3af&border_radius=14"/>

</div>
